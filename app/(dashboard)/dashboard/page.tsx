...
function TeacherDashboard({ profile }: { profile: any }) {
  ...
  const totalStudents = safeClasses.reduce((sum: number, c: any) => sum + (c.students?.length || 0), 0);
  const paidStudents = safeClasses.reduce((sum: number, c: any) => 
    sum + (c.students?.filter((s: any) => s.payment_status).length || 0), 0);
